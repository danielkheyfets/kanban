...
namespace IntroSE.Kanban.Backend.ServiceLayer
{
    ...
    public class Service : IService
    {
        ...
        public Service()
        {
            ...
            us = new UserService(loginData);
            bs = new BoardService(loginData);
        ...
        {
            Response userRP = us.LoadData();
            Response boardRP = bs.LoadData();
            Boolean Usuccess = userRP.ErrorOccured; //userLoad
            Boolean Bsuccess = boardRP.ErrorOccured; // boardLoad
            if (!Usuccess & !Bsuccess) //loadData succeeded
                return new Response();
            else
            {
                if (!(Usuccess & Bsuccess)) //both failed
                    return new Response(userRP.ErrorMessage + "and" + boardRP.ErrorMessage);
                else if (Usuccess) // userLoad succeeded, board failed
                    return userRP;
                else return boardRP;
            }
        }
        ...
        {
            String message = "";
            Response user = us.DeleteData();
            Response board = bs.DeleteData();
            if (user.ErrorOccured) // user deletion failed
                message = message + user.ErrorMessage;
            if (board.ErrorOccured)
            {
                message = message + board.ErrorMessage;
            }
            if (message.Equals(""))
                return new Response();
        ...
        {
            Response rp = us.Register(email, password, nickname);
            if (!rp.ErrorOccured) // if new user
                bs.CreateBoard(email);
            return rp;
        }
        ...
        {
            string emailHostOfAssignee = us.GetHostEmail(emailAssignee);
            return bs.AssignTask(email, emailHostOfAssignee, columnOrdinal, taskId, emailAssignee);
        }
        ...
        {
           // string _email = us.GetHostEmail(email);
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.LimitColumnTasks(email, columnOrdinal, limit, emailHost);
        }
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.ChangeColumnName(email, columnOrdinal, newName, emailHost);
        }
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.RemoveColumn(email, columnOrdinal, emailHost);
        }
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.AddColumn(email, columnOrdinal, Name, emailHost);
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.MoveColumnRight(email, columnOrdinal, emailHost);
        ...
        {
            string emailHost = us.GetHostEmail(email);
            return bs.MoveColumnLeft(email, columnOrdinal, emailHost);
